...
# rofi-pass with Aditya Shakya theming
##
## Adapted from:
...
_rofi() {
	rofi -theme "${dir}/${theme}.rasi" -matching fuzzy -no-auto-select -i "$@"
}
...
"wl-clipboard") ;;
*)
	echo >&2 "Invalid clipboard backend '$clipboard_backend', falling back to xclip"
...
"wtype") ;;
*)
	echo >&2 "Invalid backend '$backend', falling back to xdotool"
	backend=xdotool
	;;
esac
...
doClip() {
	...
	"both")
		${clip_in_primary}
		${clip_out_primary} | ${clip_in_clipboard}
		;;
	esac
}
...
openURL() {
	checkIfPass
	$BROWSER "$(PASSWORD_STORE_DIR="${root}" pass "$selected_password" | grep "${URL_field}: " | gawk '{sub(/:/,"")}{print $2}1' | head -1)"
	exit
	clearUp
}
...
copyPass() {
	...
	if [[ $notify == "true" ]]; then
		(
			...
			printf '%s' "" | ${clip_in_clipboard} | notify-send "rofi-pass" "Clipboard cleared"
		) &
	elif [[ $notify == "false" ]]; then
		(
			sleep $clip_clear
...
typeMenu() {
	if [[ -n $default_do ]]; then
		if [[ $default_do == "menu" ]]; then
			...
			typefield=$({
				printf '%s' "${AUTOTYPE_field}"
				printf '%s\n' "${keys[@]}" | sort
			} | _rofi -dmenu -p "Choose Field to type > ")
			typefield_exit=$?
...
main() {
	...
	if [[ -n "$xdotool_delay" ]]; then
		echo >&2 "Setting 'xdotool_delay' is deprecated. Please update your configuration to use 'type_delay' instead"
